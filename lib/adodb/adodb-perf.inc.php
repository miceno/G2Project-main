...
<table border=1 bgcolor=white><tr><td><b>Avg Time</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
	...
	public function SuspiciousSQL($numsql = 10) {
		return self::_SuspiciousSQL($numsql);
	}

	public function ExpensiveSQL($numsql = 10) {
		return self::_ExpensiveSQL($numsql);
	}
...
<input type="submit" value=" Run SQL Below " name="RUN"><input type=hidden name=do value=dosql>
  ...
  <tr>
  <td colspan=2><textarea rows=<?php echo $rows; ?> name="sql" cols="80"><?php echo htmlspecialchars($sql); ?></textarea>
  </td>
  </tr>
 </table>
</form>
		...
		foreach ($sqla as $sqls) {
			...
			if ($rs && is_object($rs) && !$rs->EOF) {
				...
				while ($rs->NextRecordSet()) {
					echo '<table width=98% bgcolor=#C0C0FF><tr><td>&nbsp;</td></tr></table>';
					rs2html($rs);
				}
			} else {
